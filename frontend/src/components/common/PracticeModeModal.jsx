...
import { X, Send, ShieldCheck, Building2, Tag, CheckCircle2, AlertTriangle, XCircle, RotateCcw, Award, Mic, MicOff } from 'lucide-react';
  ...
  const handleEvaluateAnswer = async (e) => {
    ...
    try {
      ...
      if (data.verdict === 'pass') {
        ...
        if (onQuestionCompleted) onQuestionCompleted(question.id, data);
      }
    } catch (err) {
            ...
            <div>
              <div className="flex items-center justify-between mb-1">
                <label className="block text-xs font-bold text-slate-700 dark:text-slate-300">
                  Type or Speak Your Answer (STAR Framework: Situation, Task, Action, Result)
                </label>

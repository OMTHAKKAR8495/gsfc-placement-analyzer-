...
export const GENUINE_GSFC_STUDENTS = [
  {
    rank: 1,
...
export default function LeaderboardTab({ currentStudent, onOpenBadgesModal }) {
  ...
  const [leaderboard, setLeaderboard] = useState(GENUINE_GSFC_STUDENTS);
  ...
  const [summary, setSummary] = useState({
    ...
    highest_streak: 12,
    is_anonymous: false,
    student_name: currentStudent?.name || 'Om Thakkar',
    nickname: 'Om T. (CSE)'
  });
  ...
  const fetchLeaderboard = async () => {
    ...
    // 1. Immediately apply smart filtering on genuine cohort
    let localFiltered = GENUINE_GSFC_STUDENTS;
    if (department && department !== 'All') {
      ...
      localFiltered = localFiltered.filter(s => {
        ...
        if (dLower.includes('chem')) return prog.includes('chem') || br.includes('chem');
        if (dLower.includes('mech')) return prog.includes('mech') || br.includes('mech');
        if (dLower.includes('it') || dLower.includes('information')) return prog.includes('it') || br.includes('information');
        if (dLower.includes('mba') || dLower.includes('management')) return prog.includes('mba') || br.includes('business');
        if (dLower.includes('fire') || dLower.includes('safety')) return prog.includes('fire') || br.includes('safety');
        if (dLower.includes('ece') || dLower.includes('electronics')) return prog.includes('ece') || br.includes('electronics');
    ...
    if (year && year !== 'All') {
      ...
      if (!isNaN(yNum)) {
        localFiltered = localFiltered.filter(s => s.passing_year === yNum || s.roll_number.startsWith(String(yNum).slice(2)));
      }
    }

    const localRanked = localFiltered.map((item, idx) => ({ ...item, rank: idx + 1 }));
    setLeaderboard(localRanked);

    // 2. Try fetching updated server state
    try {
      ...
      console.warn('Notice: Loaded genuine GSFC University student leaderboard records.');
    } finally {
      setLoading(false);
    }
  };

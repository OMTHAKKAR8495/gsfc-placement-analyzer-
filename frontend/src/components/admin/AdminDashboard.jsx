              ...
              <table className="w-full text-left border-collapse text-xs">
                <thead>
                  <tr className="bg-slate-100/90 text-slate-700 border-b border-slate-200 text-[10px] uppercase tracking-wider font-black">
                    ...
                    <th className="py-4 px-5">Outside Recruiter Contact</th>
                    <th className="py-4 px-5">Subscription & Payment Tier</th>
                    <th className="py-4 px-5">Hiring Drives & Applicants</th>
                    <th className="py-4 px-5">TPC Partner Status</th>
                    ...
                    <tr key={comp.id} className="hover:bg-slate-50/80 transition-all">
                      <td className="py-4 px-5 font-black text-slate-900">
                        <div className="flex items-center gap-3">
                          <img
                            ...
                            className="w-10 h-10 rounded-2xl object-contain bg-slate-50 p-1 border border-slate-200 shrink-0"
                          />
                          <div>
                            <div className="text-sm font-black text-slate-900">{comp.company_name}</div>
                            <div className="text-[11px] text-slate-500 font-medium">{comp.industry || 'Technology'}</div>
                            {comp.website && (
                              <a href={comp.website} target="_blank" rel="noreferrer" className="text-[10px] text-blue-700 hover:underline font-bold inline-flex items-center gap-1 mt-0.5">
                      ...
                      <td className="py-4 px-5">
                        <div className="text-slate-900 font-bold flex items-center gap-1.5">
                          ...
                          <span>{comp.email || comp.contact_email || 'recruiter@company.com'}</span>
                        </div>
                        <div className="mt-1 flex items-center gap-2">
                          <a
                            href={`https://wa.me/919558413347?text=Hello%20${encodeURIComponent(comp.company_name)},%20this%20is%20GSFC%20University%20Training%20%26%20Placement%20Cell.`}
                            target="_blank"
                            rel="noreferrer"
                            className="px-2 py-0.5 bg-emerald-50 hover:bg-emerald-100 border border-emerald-200 text-emerald-800 rounded-lg text-[10px] font-black inline-flex items-center gap-1"
                          >
                          ...
                          {comp.registered_at && (
                            <span className="text-[10px] text-slate-400">
                              Joined {new Date(comp.registered_at).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' })}
                            </span>
                          )}
                        </div>
                      </td>
                      ...
                      <td className="py-4 px-5">
                        {comp.active_plan_name ? (
                          <div className="space-y-1">
                            <span className="px-2.5 py-1 bg-amber-100 text-amber-900 border border-amber-300 text-[11px] font-black rounded-xl inline-flex items-center gap-1">
                              <Crown className="w-3 h-3 text-amber-600" />
                              <span>{comp.active_plan_name}</span>
                            </span>
                            {comp.last_paid_amount && (
                              <div className="text-[10px] text-emerald-700 font-bold">
                                Paid: ₹{comp.last_paid_amount.toLocaleString('en-IN')}
                              </div>
                            )}
                            {comp.last_receipt_number && (
                              <div className="text-[10px] text-slate-500 font-mono">
                                Rec: {comp.last_receipt_number}
                              </div>
                            )}
                          </div>
                        ) : (
                          <span className="px-2.5 py-1 bg-slate-100 text-slate-600 border border-slate-200 text-[10px] font-bold rounded-xl inline-flex items-center gap-1">
                            <Lock className="w-3 h-3 text-slate-400" />
                            <span>No Active Plan (Free Tier)</span>
                          </span>
                        )}
                      </td>
                      ...
                      <td className="py-4 px-5">
                        <div className="space-y-1.5">
                          <button
                            ...
                            className="px-3 py-1 bg-indigo-50 hover:bg-indigo-100 border border-indigo-200 text-indigo-900 font-black text-xs rounded-xl cursor-pointer transition-all hover:scale-105 inline-flex items-center gap-1.5 shadow-sm"
                            title="Click to view & delete specific individual drives for this company"
                          >
                            <span>💼 {comp.posted_drives_count || 0} Drives</span>
                            <span className="text-[10px] text-indigo-600 font-mono">Manage ▾</span>
                          </button>
                          <div className="text-[10px] text-slate-600 font-bold">
                            📥 {comp.total_applicants_count || 0} Student Applicants
                          </div>
                        </div>
                      </td>
                      ...
                      <td className="py-4 px-5">
                        {comp.approved ? (
                          <span className="px-3 py-1 bg-emerald-100 text-emerald-800 border border-emerald-300 text-xs font-black rounded-xl inline-flex items-center gap-1 shadow-sm">
                            <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600" /> APPROVED ✅
                          </span>
                        ) : (
                          <span className="px-3 py-1 bg-amber-100 text-amber-900 border border-amber-300 text-xs font-black rounded-xl inline-flex items-center gap-1 shadow-sm">
                      ...
                      <td className="py-4 px-5 text-right space-x-2">
                        {!comp.approved ? (
                          <button
                            ...
                            className="py-1.5 px-3 bg-emerald-700 hover:bg-emerald-600 text-white rounded-xl text-xs font-black transition-all shadow-sm cursor-pointer hover:scale-105"
                          >
                            Approve
                          </button>
                        ) : (
                          <button
                            ...
                            className="py-1.5 px-2.5 bg-amber-100 hover:bg-amber-200 text-amber-900 border border-amber-300 rounded-xl text-[11px] font-bold transition-all cursor-pointer"
                          >
                            Revoke
                          </button>
                        )}

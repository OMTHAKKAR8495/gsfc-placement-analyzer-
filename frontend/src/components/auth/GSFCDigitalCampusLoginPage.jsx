    ...
    if (window.google?.accounts?.oauth2 && googleClientId) {
      try {
        const tokenClient = window.google.accounts.oauth2.initTokenClient({
          ...
          error_callback: (err) => {
            console.error('Google OAuth2 popup error:', err);
            try {
        ...
        console.warn('Google OAuth2 token client init failed, trying GIS prompt:', e);
      }
    }
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 flex flex-col justify-between font-sans relative overflow-x-hidden select-none">
      ...
      <main className="flex-1 flex flex-col items-center justify-center px-4 py-4 sm:py-6 z-20 max-w-xl mx-auto w-full my-auto">
        ...
        <div className="w-full bg-white/95 backdrop-blur-xl rounded-2xl shadow-[0_20px_60px_rgba(15,23,42,0.16)] border border-white/80 p-6 sm:p-7 space-y-4">
          ...
          <div className="pt-2">
            <div id="google-signin-btn-container" className="w-full flex justify-center min-h-[42px]" />
            <button
              type="button"
              id="custom-google-auth-btn"
              onClick={triggerGoogleSignIn}
              disabled={loading}
              className="w-full py-2.5 px-3 bg-white hover:bg-slate-50 active:bg-slate-100 text-slate-700 border border-slate-300 rounded-lg text-xs font-bold flex items-center justify-center gap-2.5 transition-colors cursor-pointer shadow-2xs mt-2"
            >

...
import { 
  ...
  Shield, Check, Volume2, VolumeX, ShieldAlert, Sparkles, SwitchCamera, Upload
} from 'lucide-react';
          ...
          <div className="flex items-center gap-2">
            ...
            {!isScanning ? (
              <div className="flex items-center gap-2">
                ...
                <label className="px-3.5 py-2 bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 rounded-xl text-xs font-black shadow-md flex items-center gap-2 transition-all cursor-pointer">
                  ...
                  <input
                    ...
                    onChange={async (e) => {
                      ...
                      try {
                        ...
                        setCameraError('No QR code detected in the selected image. Please upload a clear photo of the pass QR code.');
                      }
                    }}
                  />
                </label>
        ...
        {cameraError && (
          <div className="p-4 bg-amber-500/10 border border-amber-500/30 rounded-2xl text-xs text-amber-200 space-y-2">
            ...
            <p className="text-[11px] text-slate-300">
              Your browser is currently blocking camera access. To use the live camera:
            </p>
            <ol className="list-decimal list-inside text-[11px] text-slate-300 space-y-1 font-medium pl-1">
              <li>Click the <strong>🔒 View site information / Tune icon</strong> at the left of your browser address bar.</li>
              <li>Set <strong>Camera</strong> permissions to <strong>"Allow"</strong>.</li>
              <li>Click <strong>"Launch Camera"</strong> above, or simply use <strong>Upload QR Image</strong> / <strong>Manual Token</strong> below.</li>
            </ol>
          </div>
        )}
        ...
        <div className="space-y-2">
          <form onSubmit={handleManualSubmit} className="flex gap-2">
            ...
            <button
              ...
              className="px-4 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-xs font-black border border-indigo-500 flex items-center gap-1.5 cursor-pointer disabled:opacity-40 transition-all shadow-md"
            >
              {loadingLookup ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <Search className="w-3.5 h-3.5 text-amber-300" />}
              <span>Lookup Pass</span>
            </button>
          </form>

          {/* Quick Demo Test Tokens */}
          <div className="flex items-center gap-2 flex-wrap text-[11px]">
            <span className="text-slate-400 font-bold flex items-center gap-1">
              <Sparkles className="w-3 h-3 text-amber-400" /> Test Samples:
            </span>
            <button
              type="button"
              onClick={() => {
                setManualToken('GSFC-PASS-STU-24BT04171');
                handleLookupToken('GSFC-PASS-STU-24BT04171');
              }}
              className="px-2.5 py-1 bg-slate-800 hover:bg-slate-700 text-blue-300 border border-slate-700 rounded-lg font-mono font-bold cursor-pointer transition-all hover:scale-105"
            >
              🎓 Om Thakkar (Student)
            </button>
            <button
              type="button"
              onClick={() => {
                setManualToken('GSFC-PASS-ANV-101');
                handleLookupToken('GSFC-PASS-ANV-101');
              }}
              className="px-2.5 py-1 bg-slate-800 hover:bg-slate-700 text-emerald-300 border border-slate-700 rounded-lg font-mono font-bold cursor-pointer transition-all hover:scale-105"
            >
              🎟️ Kavya Sharma (MSU)
            </button>
            <button
              type="button"
              onClick={() => {
                setManualToken('GSFC-PASS-ANV-102');
                handleLookupToken('GSFC-PASS-ANV-102');
              }}
              className="px-2.5 py-1 bg-slate-800 hover:bg-slate-700 text-purple-300 border border-slate-700 rounded-lg font-mono font-bold cursor-pointer transition-all hover:scale-105"
            >
              🎟️ Harshil Patel (Parul)
            </button>
          </div>
        </div>
      </div>

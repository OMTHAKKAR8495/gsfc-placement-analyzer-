...
function weightedSample(pool, studentProfile, targetCount) {
  ...
  const weakCategories = (studentProfile?.weakCategories || ['System Design']).map(c => c.toLowerCase());
  ...
  const scoredPool = pool.map((q, idx) => {
    ...
    // AI Evaluation Weak Category Boost (+2.0 weight) -> Adaptive practice prioritization!
    const qCat = (q.category || '').toLowerCase();
    if (weakCategories.some(wc => qCat.includes(wc) || wc.includes(qCat))) {
      score += 2.0;
    }

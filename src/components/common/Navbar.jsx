...
import { User, Building2, ShieldCheck, LogOut, LogIn } from 'lucide-react';

export default function Navbar({ currentUser, activeRole, onRoleSwitch, onOpenAuth, onLogout }) {
  return (
    <header className="sticky top-0 z-40 w-full glass-panel border-b border-slate-200/90 bg-white/90 backdrop-blur-xl shadow-sm">
      <div className="max-w-7xl mx-auto px-3 sm:px-6 lg:px-8 min-h-[4.5rem] py-2 flex flex-wrap items-center justify-between gap-2 sm:gap-4">
        ...
        {/* Segmented Role Navigation Bar (Fully Responsive) */}
        <div className="flex items-center bg-slate-100/90 p-1 sm:p-1.5 rounded-2xl border border-slate-200/80 shadow-inner overflow-x-auto max-w-full">
          <button
            onClick={() => onRoleSwitch('student')}
          ...
          {currentUser ? (
            <div className="flex items-center gap-2 sm:gap-3">
              <div className="text-right hidden md:block">
                <div className="text-xs font-black text-slate-900">{currentUser.profile?.name || currentUser.email}</div>
                <div className="text-[10px] font-black text-blue-900 uppercase tracking-wider flex items-center justify-end gap-1">
              ...
              <button
                onClick={onLogout}
                className="p-2 rounded-xl bg-slate-100 hover:bg-red-50 hover:text-red-600 text-slate-600 border border-slate-200 transition-all text-xs font-bold"
                title="Sign Out"
              >
            ...
            <button
              onClick={onOpenAuth}
              className="flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2 sm:py-2.5 rounded-xl bg-gradient-to-r from-blue-900 via-indigo-900 to-amber-600 hover:from-blue-800 hover:to-amber-500 text-white font-black text-[11px] sm:text-xs shadow-lg shadow-blue-900/20 transition-all shrink-0 whitespace-nowrap"
            >

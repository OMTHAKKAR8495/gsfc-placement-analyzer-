...
router.get('/', async (req, res) => {
  try {
    ...
    const rawFairs = await db.prepare(query).all(...params);
    const fairs = [];
    for (const fair of rawFairs) {
...
router.post('/', AuthRateLimiter.generalApiLimiter, async (req, res) => {
  try {
    ...
    const createdFair = await db.prepare('SELECT * FROM job_fairs WHERE id = ?').get(fairId);
...
router.put('/:id', async (req, res) => {
  try {
    ...
    const existing = await db.prepare('SELECT id FROM job_fairs WHERE id = ?').get(id);
    if (!existing) {
      return res.status(404).json({ error: 'Job fair event not found.' });
    }
    ...
    const updated = await db.prepare('SELECT * FROM job_fairs WHERE id = ?').get(id);
...
router.delete('/:id', async (req, res) => {
  try {
    const { id } = req.params;
    await db.prepare('DELETE FROM job_fairs WHERE id = ?').run(id);
...
router.post('/:id/add-company', async (req, res) => {
  try {
...
router.delete('/:id/remove-company/:requirementId', async (req, res) => {
  try {
...
router.post('/:id/register', AuthRateLimiter.generalApiLimiter, async (req, res) => {
  try {
    const { id: fairId } = req.params;
    ...
    const fair = await db.prepare('SELECT * FROM job_fairs WHERE id = ?').get(fairId);
    if (!fair) {
      return res.status(404).json({ error: 'Job fair event not found.' });
    }

    let resolvedStudentId = student_id;
    let student = await db.prepare('SELECT id FROM student_profiles WHERE id = ? OR user_id = ?').get(student_id, student_id);
    if (student) {
      ...
      const validUser = await db.prepare('SELECT id FROM users WHERE id = ?').get(student_id);
...
router.get('/student/registrations', async (req, res) => {
  try {
    ...
    const registeredFairIds = await db.prepare(`
...
router.get('/company/my-fairs', async (req, res) => {
  try {

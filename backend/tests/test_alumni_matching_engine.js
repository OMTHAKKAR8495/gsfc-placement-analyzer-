...
async function runAlumniTests() {
  ...
  const updatedSlot = await db.prepare('SELECT * FROM alumni_mentorship_slots WHERE id = ?').get(slotId);
  assert(Number(updatedSlot.is_booked) === 1, 'Slot must be marked as booked');
  ...
  assert(Number(reviewStats.count) >= 1, 'Review must be recorded');
  assert(Number(reviewStats.avg_rating) === 5, 'Average rating must be 5.0');
  console.log(`   ✅ Verified ${reviewStats.count} review. Mentor Average Rating: ★ ${Number(reviewStats.avg_rating).toFixed(1)}/5.0`);

  // Clean up
  await db.prepare('DELETE FROM alumni_mentor_reviews WHERE id = ?').run(reviewId);
  await db.prepare('DELETE FROM alumni_mentorship_slots WHERE id = ?').run(slotId);
  await db.prepare('DELETE FROM alumni_profiles WHERE id = ?').run(testAlumniId);
